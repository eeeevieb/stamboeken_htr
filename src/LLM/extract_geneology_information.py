...
def zero_shot(input_file, description_file):
    ...
    results = [
        line.find(
            "ns:TextEquiv/ns:PlainText", namespaces={"ns": "http://schema.primaresearch.org/PAGE/gts/pagecontent/2013-07-15"}
        ).text
        for line in result
        if line is not None
    ]
        ...
        messages=[
            {
                ...
                + "extract the information mentioned in this following file"
                + description_file
                + ".\
