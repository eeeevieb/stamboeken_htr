...
prompt_1 = rf"Attached are two handwritten Dutch pages, \"NL-HaNA_2.10.50_1_0005_0\" and \""+"{image}"+"\".\
                ...
                \
                Q: Can you provide me with a PageXML file containing coordinates of text regions, text lines and baselines of \""+"{image}"+"\", and add labels to the text lines? It can be one of the following labels: Name, Date, Place, Orde. If the text does not fit any label, add label Text. If the text fits more than one label, add both."
...
prompt_2 = rf"The following pageXML file contains regions, textlines and baselines of the attached image."+ "{input_file}"+ "This image contains names, geneological data, date of appointment and advancement in the corps, which ship was taken, when and how discharged or died, and specialities.\
                Q: Can you add a label in the form of \"structure {{Type:[label];}}\" to the custom tag of the TextLine element with id \""+"{text_id}"+"\"? It can be one of the following labels: Name, Date, Place, Orde. If the text does not fit any label, add label Text. If the text fits more than one label, add both.\
...
if __name__ == "__main__":
    if LLAMA_1 or LLAMA_2:
        ...
        if LLAMA_1:
            ...
            for index, xml in enumerate(xml_list):
                ...
                results = prompt_llama(llama_client, prompt_1, xml_path, [im_path, '/root/Thesis/stamboeken_htr/experiment_1/stamboeken/NL-HaNA_2.10.50_1_0005_0.JPG'], textline_string, textline_id, name, 1)
    ...
    if GEMINI_1 or GEMINI_2:
        ...
        if GEMINI_1:
            ...
            for index, xml in enumerate(xml_list):
                xml_path, im_path, textline_string, textline_id, name = get_prompt_vars(xml, im_list[index])
                result = prompt_gemini(gemini_client, prompt_1,xml_path, [im_path, '/root/Thesis/stamboeken_htr/experiment_1/stamboeken/NL-HaNA_2.10.50_1_0005_0.JPG'], textline_string, textline_id, name, 1)
