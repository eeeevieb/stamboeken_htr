...
def prompt_gemini(client, prompt_template, xml, image, textline_string, textline_id, name, exp):
    ...
    with open(xml, "r") as f:
        ...
        prompt = prompt_template.format(
            input_file=input_file, text_id=textline_id, name=name, element=textline_string, image=image[0]
        )

        # Prompt Gemini
        if exp == 1:
            response = client.models.generate_content(model="gemini-2.0-flash", contents=[prompt, image[0], image[1]])
        if exp == 2:
            response = client.models.generate_content(model="gemini-2.0-flash", contents=[prompt, image[0]])
...
def prompt_llama(client, prompt_template, xml, image, textline_string, textline_id, name, exp):
    ...
    with open(xml, "r") as f:
        ...
        # Fill in prompt
        prompt = prompt_template.format(
            input_file=input_file, text_id=textline_id, name=name, element=textline_string, image=base64_image
        )
